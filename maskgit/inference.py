...
class ImageNet_class_conditional_generator():
    ...
    def create_latent_mask_and_input_tokens_for_image_editing(self, image, bbox, target_label, mask=None):
        ...
        if mask is not None:
            latent_mask = np.stack([mask]*self.maskgit_cf.eval_batch_size,0)
        else:
